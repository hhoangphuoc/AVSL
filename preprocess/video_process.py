...
def load_video(video_path, to_grayscale=True, max_frames=None):
    """
...
def extract_lip_frames(video_path, 
    ...
    try:
        ...
        # Load video frames using OpenCV, limiting frame count
        frames = load_video(video_path, to_grayscale=to_grayscale, max_frames=max_frames)
...
def batch_process_lip_videos(video_paths, output_paths, **kwargs):
    """
    Process multiple videos sequentially to extract lip videos.
    ...
    print(f"Sequential processing {total_videos} videos with GPU acceleration for face detection...")
    ...
    try:
        for i, (video_path, output_path) in enumerate(tqdm(
                zip(video_paths, output_paths), 
                desc="Processing videos", 
                total=total_videos)):
            
            # Process with additional error handling
            try:
                # Explicit garbage collection before each video
                gc.collect()
                
                # Process video
                result = extract_and_save_lip_video(video_path, output_path, **kwargs)
                ...
                if (i + 1) % 10 == 0 or i == total_videos - 1:
                    print(f"Processed {i+1}/{total_videos} videos. Success rate: {successful_count/(i+1)*100:.1f}%")
                    
            except Exception as e:
                print(f"Critical error processing video {video_path}: {str(e)}")
                results.append((False, None))
    ...
    # If original input was a dictionary, map results back to segment IDs
    if segment_ids:
        lip_results_dict = {}
        for i, segment_id in enumerate(segment_ids):
            if i < len(results):
                lip_results_dict[segment_id] = results[i]
            else:
                lip_results_dict[segment_id] = (False, None)
                
        # Add entries for videos that were skipped initially
        if isinstance(video_paths, dict):
            for segment_id in video_paths.keys():
