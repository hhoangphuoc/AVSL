...
#                           Basic Blocks
# Including: 
# 3x3 Convolutional Layer: a 3x3 convolutional layer with padding 1
# Downsample Basic Block: a Convolutional Layer followed by a BatchNorm Layer
# Downsample Basic Block v2: a Convolutional Layer followed by a BatchNorm Layer and an AvgPool Layer
# ================================================================================
...
class ResNetEncoderLayer(nn.Module):
    """
    The ResNetEncoderLayer implementation for AV-HuBERT model. This component acts
    as the visual frontend for the model, which is used to extract the visual features from the video frames.

    The ResNetEncoderLayer is a 3D convolutional frontend followed by a ResNet backbone.
    The implementation is inherited from:
    https://github.com/facebookresearch/av_hubert/blob/main/avhubert/resnet.py

    Args:
        relu_type (str): The type of the activation function.
        weights (str): The path to the weights file (can be None)
    """
    def __init__(self, 
                 frontend_out_channels=64,
                 backbone_out_channels=512,
                 relu_type='prelu',
                 weights=None):
        super(ResNetEncoderLayer, self).__init__()
        self.frontend_nout = frontend_out_channels
        self.backend_out = backbone_out_channels

        # NOTE: 3D Convolutional Frontend for processing video frames
        ...
        # NOTE: ResNet Backbone
        self.trunk = ResNet(BasicBlock, [2, 2, 2, 2], relu_type=relu_type)

        # NOTE: Load pretrained weights if provided
    ...
    def forward(self, x):
        ...
        # Reshape for processing by the 2D ResNet
        Tnew = x.shape[2]
        x = self.threeD_to_2D_tensor(x) # (B, C, T, H, W) -> (B*T, C, H, W) 
        x = self.trunk(x)

        # Reshape back to sequence form (B, C, T)
